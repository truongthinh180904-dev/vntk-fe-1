...
(function() {
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.tagName === 'LINK' && node.href.includes("plg.datacom.vn/css")) {
                    console.log("Đã chặn CSS:", node.href);
                    node.remove();
                }
            });
        });
    ...
    // Theo dõi cả head và body (phòng trường hợp plugin inject linh tinh)
    observer.observe(document.documentElement, { childList: true, subtree: true });

    // Quét dọn 1 lần ngay khi load
    const removeCSS = () => {
        document.querySelectorAll('link[rel="stylesheet"]').forEach(link => {
            if (link.href.includes("plg.datacom.vn/css")) {
                console.log("Đã xóa CSS:", link.href);
                link.remove();
            }
        });
    };
...
const addAirlineFilter = () => {
  const targetDiv = document.querySelector('.dtc-search-input-date');
  if (!targetDiv) return false;

  // Tạo div mới
  const newDiv = document.createElement('div');
  newDiv.className = 'filter-airline dtc-horizontal-option';
  newDiv.innerHTML = `
   
<div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:15px">

  <div class="dtc-form-check" style="display:flex; align-items:center; border:2px solid #2F6AE8; border-radius:6px; padding:5px 10px; gap:5px;">
    <input class="dtc-form-check-input" type="checkbox" value="dtc-option-air-all" id="dtc-option-air-all" checked>
    <label class="dtc-form-check-label" for="dtc-option-air-all" style="margin-top:5px;">Tất cả</label>
  </div>

  <div class="dtc-form-check" style="display:flex; align-items:center; border:2px solid #2F6AE8; border-radius:6px; padding:5px 10px; gap:5px;">
    <input class="dtc-form-check-input" type="checkbox" value="VN" id="dtc-option-air-vn" checked>
    <label class="dtc-form-check-label" for="dtc-option-air-vn" style="margin-top:5px;">
      <img src="https://plg.datacom.vn/img/vn.png" alt="Vietnam Airlines" style="height:22px;">
    </label>
  </div>

  <div class="dtc-form-check" style="display:flex; align-items:center; border:2px solid #2F6AE8; border-radius:6px; padding:5px 10px; gap:5px;">
    <input class="dtc-form-check-input" type="checkbox" value="VJ" id="dtc-option-air-vj" checked>
    <label class="dtc-form-check-label" for="dtc-option-air-vj" style="margin-top:5px;">
      <img src="https://plg.datacom.vn/img/vj.png" alt="Vietjet Air" style="height:22px;">
    </label>
  </div>

  <div class="dtc-form-check" style="display:flex; align-items:center; border:2px solid #2F6AE8; border-radius:6px; padding:5px 10px; gap:5px;">
    <input class="dtc-form-check-input" type="checkbox" value="QH" id="dtc-option-air-qh" checked>
    <label class="dtc-form-check-label" for="dtc-option-air-qh" style="margin-top:5px;">
      <img src="https://plg.datacom.vn/img/qh.png" alt="Bamboo Airways" style="height:22px;">
    </label>
  </div>

  <div class="dtc-form-check" style="display:flex; align-items:center; border:2px solid #2F6AE8; border-radius:6px; padding:5px 10px; gap:5px;">
    <input class="dtc-form-check-input" type="checkbox" value="VU" id="dtc-option-air-vu" checked>
    <label class="dtc-form-check-label" for="dtc-option-air-vu" style="margin-top:5px;">
      <img src="https://plg.datacom.vn/img/vu.png" alt="Vietravel Airlines" style="height:22px;">
    </label>
  </div>
  ...
  console.log("✅ Đã thêm filter airline ngay sau dtc-search-input-date");
  return true;
};

// Chờ phần tử xuất hiện (polling)
const waitForDiv = setInterval(() => {
  if (addAirlineFilter()) clearInterval(waitForDiv);
}, 300);



function addIconToButton() {
  const buttons = document.querySelectorAll(".dtc-option-left .dtc-dropdown-btn");

  buttons.forEach((btn) => {
    if (!btn.classList.contains("icon-injected")) {
      // Kiểm tra nếu có span #txtPax (script gốc dùng để cập nhật)
      const paxSpan = btn.querySelector("#txtPax");

      // Lấy text ban đầu
      const text = paxSpan ? paxSpan.textContent.trim() : btn.textContent.trim();

      // Tạo icon theo nội dung
      let iconSVG = "";
      if (text.includes("chiều")) {
        // icon máy bay
        iconSVG = `
          <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19"
            viewBox="0 0 24 24" fill="none" stroke="currentColor"
            stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            style="margin-right:4px;margin-bottom:6px;vertical-align:middle">
            <path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z"/>
          </svg>`;
      } else if (paxSpan || text.includes("Người")) {
        // icon user
        iconSVG = `
          <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19"
            viewBox="0 0 24 24" fill="none" stroke="currentColor"
      ...
      // Thêm icon vào trước span text (hoặc trước #txtPax nếu có)
      if (paxSpan) {
        // giữ nguyên span gốc
        const wrapper = document.createElement("span");
        wrapper.innerHTML = iconSVG;
        btn.insertBefore(wrapper, paxSpan);
      } else {
        // fallback nếu không có #txtPax
        btn.innerHTML = `
      ...
      btn.classList.add("icon-injected");
    }
  });
...
const waitForButton = setInterval(() => {
  if (addIconToButton()) clearInterval(waitForButton);
}, 300);


function addCustomHeaderOption() {
  const searchOption = document.querySelector(".dtc-search-option");
  if (!searchOption) return false;

  // Kiểm tra nếu đã thêm rồi thì không thêm nữa
  if (document.querySelector(".custom-header-option")) return true;

  // Tạo div header mới
  const headerDiv = document.createElement("div");
  headerDiv.className = "custom-header-option";
  headerDiv.style.marginBottom = "12px";

  // Nội dung có thể là nút hoặc select
  headerDiv.innerHTML = `
 <div style="text-align: left; margin: 5px;">
  <h1 style="
    ...
    margin: 0;
  ">
   Tìm kiếm chuyến bay mới nhất
  </h1>
</div>

  `;

  // Gắn div này lên trước .dtc-search-option
  searchOption.parentNode.insertBefore(headerDiv, searchOption);

  return true;
}

// Polling chờ dtc-search-option có mặt
const waitForOption = setInterval(() => {
  if (addCustomHeaderOption()) clearInterval(waitForOption);
}, 300);
