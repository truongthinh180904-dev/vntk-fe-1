  ...
  import banner from "../assets/img/domestic/banner-hero-2.jpg"
  ...
  useEffect(() => {
  const CACHE_KEY = "promotion_session_2";
    ...
    try {
      ...
      if (cached) {
        ...
        if (parsed.timestamp && Date.now() - parsed.timestamp < CACHE_DURATION) {
          ...
          console.log("✅ Lấy promotion category=50 từ cache");
          setLoading(false);
          return;
        }
      }
      ...
      // console.log("✅ Lấy promotion flights từ API category=50 và lưu cache");
    } catch (err) {
      console.error("❌ Lỗi khi fetch bài viết theo danh mục:", err);
    } finally {
      setLoading(false);
    ...
    return (
     <>
      <div className="news-all py-3">
        <h1 className="fw-bold mb-4 text-uppercase text-dark">Ưu đãi Flash sale giảm giá</h1>     
        <div>
            <div>
            <div
              // ref={sliderRef}
