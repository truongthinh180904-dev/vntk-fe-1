...
const NewsAll = () => {
  ...
  const CACHE_KEY = "news_session_2";
  ...
  <div className="col-lg-7">
    <Link href={`/${highlightPost.slug}`} className="text-decoration-none text-dark">
      <div className="card border-0 shadow-sm rounded-4 overflow-hidden h-100">
        <div className="news-main-img-wrapper" style={{ width: "100%", aspectRatio: "16/9", overflow: "hidden" }}>
          <img
            ...
            style={{ 
              width: "100%", 
              height: "100%", 
              objectFit: "cover", // Giữ tỉ lệ ảnh, không bị bẹp
              display: "block" 
            }}
          />
        </div>
        <div className="card-body">
          <p className="text-primary small fw-bold mb-1">
            BLOG <span className="text-muted"><History style={{ marginLeft: "5px", color: "#777" }} size={16} /> {highlightPost.time}</span>
          </p>
          <h5 // Tăng lên h5 cho tiêu đề nổi bật
            className="fw-bold"
          ...
          <p
            ...
            style={{
              display: '-webkit-box',
              WebkitLineClamp: 3, // Giới hạn mô tả 3 dòng tránh quá dài
  ...
  <div className="col-lg-5 d-flex flex-column gap-3">
    {sidePosts.map((post, idx) => (
      ...
      >
        <div className="d-flex align-items-start gap-3 news-side-item border-bottom pb-2">
          <img
            src={post.img}
            alt={post.title}
            className="rounded-2"
            style={{
              width: '155px',      // Giảm nhẹ chiều rộng để hài hòa hơn trên mobile
              height: '105px',       // Cố định chiều cao
              flexShrink: 0,        // Tuyệt đối không cho ảnh bị co lại
              objectFit: 'cover',   // Chống bẹp hình
              backgroundColor: '#f0f0f0' // Màu nền chờ nếu ảnh lỗi
            }}
          />
          <div style={{ minWidth: 0 }}> 
            <p className="text-primary x-small fw-bold mb-1" style={{ fontSize: '0.75rem' }}>
              BLOG <span className="text-muted">{post.time}</span>
            </p>
            <h6
              className="fw-semibold small news-side-title mb-0"
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 2,
                WebkitBoxOrient: 'vertical',
